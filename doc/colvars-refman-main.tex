...
\item %
  \key{%
    lowerWallConstant}{%
    \texttt{colvar}}{%
    Lower wall force constant (\cvnamdonly{kcal/mol}\cvlammpsonly{unit of energy specified by \texttt{units}})}{%
    positive decimal}{%
    ...
    The energy unit of the constant is \cvnamdonly{kcal/mol}\cvlammpsonly{the unit of energy specified by \texttt{units}}, while the spatial unit is that of the colvar.}
...
\item %
  \key{%
    upperWallConstant}{%
    \texttt{colvar}}{%
    Upper wall force constant (\cvnamdonly{kcal/mol}\cvlammpsonly{unit of energy specified by \texttt{units}})}{%
    positive decimal}{%
...
\item %
  \keydef{%
    ...
    physical unit for this force is \cvnamdonly{kcal/mol}\cvlammpsonly{the unit of energy specified by \texttt{units}}, divided by the colvar unit.}
...
\item %
  \keydef{%
    ...
    force is \cvnamdonly{kcal/mol}\cvlammpsonly{the unit of energy specified by \texttt{units}} divided by the colvar unit.}
\end{itemize}
...
\item \keydef{fullSamples}{\texttt{abf}}{%
  ...
  {To avoid nonequilibrium effects due to large fluctuations of the force exerted along the
   ...
   between 0 and 1. If the number of samples in the current bin is higher than \texttt{fullSamples},
...
\item \keydef{maxForce}{\texttt{abf}}{%
  ...
  {This option enforces a cap on the magnitude of the biasing force effectively applied
   ...
   is \cvnamdonly{kcal/mol}\cvlammpsonly{the unit of energy specified by \texttt{units}} divided by the colvar unit (\AA{} for lengths, degrees for angles, etc.).}
...
\item %
  \keydef{%
    hillWeight}{%
    \texttt{metadynamics}}{%
    Height of each hill (\cvnamdonly{kcal/mol}\cvlammpsonly{unit of energy specified by \texttt{units}})}{%
    positive decimal}{%
    \texttt{0.01}}{%
...
\item %
  \keydef{%
    forceConstant}{%
    \texttt{harmonic}}{%
    Scaled force constant (\cvnamdonly{kcal/mol}\cvlammpsonly{unit of energy specified by \texttt{units}})}{%
    positive decimal}{%
    \texttt{1.0}}{%
...
by the \texttt{width} corresponding to biased colvar. In White and
Voth, it's scaled by the \texttt{centers}. The bias converges to a
linear bias, after which it will be the minimal possible bias. You may
also stop the simulation, take the median of the force constants
(ForceConst) found in the colvars trajectory file, and then apply a
linear bias with that constant. All the notes about units described in
sections \ref{sec:colvarbias_linear} and \ref{sec:colvarbias_harmonic}
apply here as well. {\bf This is not a valid simulation of any
particular statistical ensemble and is only an optimization algorithm
until the bias has converged}.
